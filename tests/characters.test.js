...
const dummyMovie = {
  title: 'The Lion King',
  creationDate: new Date(1994, 6, 8),
  qualification: 5,
    ...
    test('If the token is valid and the data is valid, the character is created', (done) => {
      ...
      request(app)
        ...
        .end((err, res) => {
          ...
          expect(parseInt(res.body.age)).toBe(dummyCharacter.age)
          expect(parseFloat(res.body.weight)).toBe(dummyCharacter.weight)
                ...
                .then(() => {
                  request(app)
                    ...
                    .end((err, res) => {
                      ...
                      expect(movie.qualification).toBe(movieInDB.qualification)
                      expect(movie.image).toBe(movieInDB.image)
                      done()
                    })
                })
      ...
      Character.create(dummyCharacter)
        .then((character) => {
          request(app)
            ...
            .set('Authorization', `Bearer ${authToken}`)
            .field('name', 'Mickey')
            .field('image', __dirname + '/resources/characters/mickey.png')
            .end((err, res) => {
              expect(res.status).toBe(200)
